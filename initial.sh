ln -sfn $HOME/dotfiles/gitconfig $HOME/.gitconfig
ln -sfn $HOME/dotfiles/vim $HOME/.config/nvim
ln -sfn $HOME/dotfiles/vim $HOME/.vim
ln -sfn $HOME/dotfiles/vim/init.vim $HOME/.vimrc
ln -sfn $HOME/dotfiles/i3 $HOME/.i3
ln -sfn $HOME/dotfiles/dunst $HOME/.config/dunst
ln -sfn $HOME/dotfiles/alacritty $HOME/.config
ln -sfn $HOME/dotfiles/kitty/custom.conf $HOME/.config/kitty
ln -sfn $HOME/dotfiles/tmux.conf $HOME/.tmux.conf
ln -sfn $HOME/dotfiles/vscode/settings.json $HOME/Library/Application\ Support/Code\ -\ Insiders/User/settings.json
ln -sfn $HOME/dotfiles/vscode/snippets $HOME/Library/Application\ Support/Code\ -\ Insiders/User
ln -sfn $HOME/dotfiles/.hyper.js $HOME/.hyper.js
